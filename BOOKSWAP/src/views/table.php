...
<!--
    View: table.php
    Scopo: lista principale degli annunci, include:
    - ricerca client-side e filtri server-side
    - griglia di card che vengono popolate da $table fornito dal controller
    - modal per il dettaglio annuncio con gallery e azioni (preferiti/ordina)
    Javascript in fondo gestisce filtri client-side e la modal.
-->
<style>
    /* BARRA RICERCA */
    .search-wrap {
    ...
    .filtri-form input[type="number"]:focus {
        border-color: var(--gold);
    }
    ...
    .modal-btn-preferiti:hover {
        background: var(--gold2);
    }

    .modal-btn-preferiti.nascosto {
        display: none;
    }
...
<form class="filtri-form" method="GET" action="index.php">
    ...
    <div>
        ...
        <select name="id_materia">
            ...
            <?php foreach ($materie as $m): ?>
                <option value="<?= $m['id_materia'] ?>" <?= (($_GET['id_materia'] ?? '') == $m['id_materia']) ? 'selected' : '' ?>>
    ...
    <div>
        ...
        <input type="number" name="prezzo_max" min="0" step="0.50" placeholder="es. 20"
    ...
    <?php if (!empty($table)): ?>
        <?php foreach ($table as $a): ?>
            <div class="book-card" data-titolo="<?= htmlspecialchars(strtolower($a['titolo'] ?? '')) ?>"
                ...
                data-id-libro="<?= $a['id_libro'] ?>" data-id-creatore="<?= $a['id_creatore'] ?>"
                ...
                data-immagini="<?= htmlspecialchars($a['immagini'] ?? '') ?>" onclick="apriModal(this)">

                <?php if (isset($_SESSION['id_utente'])): ?>
                    <a href="index.php?page=preferiti&action=store&id_annuncio=<?= $a['id_annuncio'] ?>" class="heart-link"
                        title="Aggiungi ai preferiti" onclick="event.stopPropagation()">❤️</a>
                <?php endif; ?>
                ...
                <?php if (isset($_SESSION['id_utente']) && $_SESSION['id_utente'] != $a['id_creatore']): ?>
                    <a href="index.php?page=ordini&action=ordina&id_annuncio=<?= $a['id_annuncio'] ?>" class="btn-ordina-card"
    ...
    <div class="modal-box" role="dialog" aria-modal="true">
        <div class="modal-inner">
            ...
            <div class="modal-dettagli">
                ...
                <a href="#" class="modal-btn-preferiti" id="modal-btn-preferiti">&#10084;&#65039; Aggiungi ai
                    preferiti</a>
                <a href="#" class="modal-btn-ordina" id="modal-btn-ordina"
