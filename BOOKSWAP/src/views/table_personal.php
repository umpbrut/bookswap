...
<style>
    ...
    .modal-btn-modifica:hover,
    .modal-btn-elimina:hover {
        opacity: 0.85;
    }
</style>
...
<div class="annunci-grid">
    <?php if (!empty($attivi)): ?>
        <?php foreach ($attivi as $a):
            ...
            ?>
            <div class="book-card" data-titolo="<?= htmlspecialchars(strtolower($a['titolo'] ?? '')) ?>"
                data-autore="<?= htmlspecialchars(strtolower($a['autore'] ?? '')) ?>" data-id="<?= $id ?>"
                data-id-libro="<?= $libro ?>" data-materia="<?= htmlspecialchars($a['materia'] ?? '') ?>"
                ...
                data-data="<?= htmlspecialchars($a['data'] ?? '') ?>" data-ora="<?= htmlspecialchars($a['ora'] ?? '') ?>"
                data-luogo="<?= htmlspecialchars($a['luogo'] ?? '') ?>"
                data-stato="<?= htmlspecialchars($a['stato'] ?? 'Disponibile') ?>"
                data-immagini="<?= htmlspecialchars($a['immagini'] ?? '') ?>" onclick="apriModalP(this)">
                ...
                <div class="card-actions" onclick="event.stopPropagation()">
                    ...
                    <a href="index.php?page=annunci&action=destroy&id_annuncio=<?= $id ?>" class="btn-elimina"
            ...
            <?php foreach ($conclusi as $a): ?>
                <tr>
                    ...
                    <td style="color:var(--gold);font-weight:500;">€
                        <?= number_format($a['prezzo_vendita'] ?? 0, 2, ',', '.') ?>
                    </td>
                    <?php
                    $s = $a['stato'] ?? '';
    ...
    <div class="modal-box" role="dialog" aria-modal="true">
        <div class="modal-inner">
            ...
            <div class="modal-dettagli">
                ...
                <div class="modal-info-riga"><span class="modal-info-label">Stato annuncio</span><span
                        class="modal-info-val" id="modal-stato-p"></span></div>
                <div class="modal-info-riga"><span class="modal-info-label">Data incontro</span><span
                        class="modal-info-val" id="modal-data-p"></span></div>
                <div class="modal-info-riga"><span class="modal-info-label">Ora</span><span class="modal-info-val"
                        id="modal-ora-p"></span></div>
                <div class="modal-info-riga"><span class="modal-info-label">Luogo</span><span class="modal-info-val"
                        id="modal-luogo-p"></span></div>
                <div class="modal-divider"></div>
...
<script>
    // JavaScript: gestione modale di dettaglio per gli annunci nella pagina personale.
    // imgsP: array di URL immagini per la card selezionata.
    let imgsP = [];
    let imgCorrP = 0; // indice immagine corrente

    /**
     * Apri la modale e popola i campi con i dati presenti negli attributi data-* della card.
     * Ogni card contiene i dataset usati qui (titolo, autore, prezzo, immagini ecc.).
     */
    function apriModalP(card) {
        ...
        // Costruisco gli URL per i pulsanti di modifica/elimina usando gli id della card.
        let id = card.dataset.id;
        let libro = card.dataset.idLibro || card.dataset.idLibro === '' ? card.dataset.idLibro : '';
        ...
        // Preparo le immagini (se presenti) e inizializzo la gallery.
        imgsP = card.dataset.immagini ? card.dataset.immagini.split(',') : [];
        imgCorrP = 0;
        costruisciThumbsP();
        aggiornaImmagineP();

        // Mostra modale e blocca scroll della pagina.
    ...
    /**
     * Costruisce la riga di thumbnail nella modale.
     * Nota: manteniamo sempre 3 slot (placeholder se non esistono immagini).
     */
    function costruisciThumbsP() {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            // closure per associare correttamente l'indice all'onclick
            ...
            if (imgsP[i]) {
                ...
                // Placeholder visivo per ricordare gli slot non riempiti
                div.textContent = '⊡';
            }
    ...
    /**
     * Aggiorna l'immagine principale della modale in base a imgCorrP.
     * Nasconde il placeholder se esiste un'immagine, altrimenti mostra il placeholder.
     */
    function aggiornaImmagineP() {
        ...
        // Aggiorna classe 'attiva' sulle thumbs
    ...
    /**
     * Cambia immagine in avanti o indietro (dir: +1 / -1). Supporta wrap-around.
     */
    ...
    /**
     * Se si clicca sull'overlay (fuori dalla box) chiude la modale.
     */
    ...
    // Ascolta Escape per chiudere la modale dalla tastiera.
