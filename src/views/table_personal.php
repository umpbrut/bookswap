...
<style>
    ...
    .modal-btn-modifica, .modal-btn-elimina {
    ...
    <div class="modal-box" role="dialog" aria-modal="true">
        <div class="modal-inner">
            ...
            <div class="modal-gallery">
                <div class="gallery-main" id="modal-img-main-p">
                    <button class="gallery-arrow prev" onclick="cambiaImmaginep(-1)">&#8249;</button>
                    <span id="modal-placeholder-icon-p">&#9413;</span>
                    <button class="gallery-arrow next" onclick="cambiaImmaginep(1)">&#8250;</button>
                </div>
                <div class="gallery-thumbs" id="gallery-thumbs-p">
                    <div class="gallery-thumb attiva" onclick="selezionaThumbP(0)">&#9413;</div>
                    <div class="gallery-thumb" onclick="selezionaThumbP(1)">&#9413;</div>
                    <div class="gallery-thumb" onclick="selezionaThumbP(2)">&#9413;</div>
                </div>
            </div>

            <div class="modal-dettagli">
                <button class="modal-chiudi" onclick="chiudiModalP()" aria-label="Chiudi">&#10005;</button>
                ...
                <div class="modal-info-riga">
                    ...
                    <span class="modal-info-val" id="modal-stato-p"></span>
                </div>
                <div class="modal-info-riga">
                    <span class="modal-info-label">Data incontro</span>
                    <span class="modal-info-val" id="modal-data-p"></span>
                </div>
                <div class="modal-info-riga">
                    <span class="modal-info-label">Ora</span>
                    <span class="modal-info-val" id="modal-ora-p"></span>
                </div>
                <div class="modal-info-riga">
                    <span class="modal-info-label">Luogo</span>
                    <span class="modal-info-val" id="modal-luogo-p"></span>
                </div>
...
<script>
    ...
    function apriModal(card) {
        ...
        document.getElementById('modal-prezzo-p').textContent     = 'EUR ' + card.dataset.prezzo;
        ...
        let libro = card.dataset.idLibro;
        document.getElementById('modal-btn-modifica').href =
            'index.php?page=annunci&action=update&id_annuncio=' + id + '&id_libro=' + libro;
        document.getElementById('modal-btn-elimina').href =
            'index.php?page=annunci&action=destroy&id_annuncio=' + id;
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') chiudiModalP();
    });

    function cambiaImmaginep(dir) {
        immagineCorrP = (immagineCorrP + dir + NUM_IMG_P) % NUM_IMG_P;
        aggiornaGalleriaP();
    }

    function selezionaThumbP(idx) {
        immagineCorrP = idx;
        aggiornaGalleriaP();
    }

    function aggiornaGalleriaP() {
        document.querySelectorAll('#gallery-thumbs-p .gallery-thumb').forEach(function(t, i) {
            t.classList.toggle('attiva', i === immagineCorrP);
        });
    }
</script>