...
<!--
    View: table_preferiti.php
    Scopo: mostra la griglia dei preferiti dell'utente autenticato.
    Note: include stili per la griglia, card e la modal; lo script in fondo
    gestisce l'apertura/chiusura della modal e la gallery delle immagini.
-->
<style>
    .annunci-grid {
        display: grid;
    ...
    /* MODIFICATO: Rimossa l'opacity: 0.45 per togliere la patina dalle card */
    .card-img-wrap {
        width: 100%;
        aspect-ratio: 3/4;
    ...
    /* MODIFICATO: Rimossa l'opacity: 0.5 per togliere la patina dentro la modal */
    .gallery-main {
        width: 100%;
        aspect-ratio: 3/4;
        background: linear-gradient(135deg, var(--bg2), var(--bg3));
    ...
    /* MODIFICATO: Cambiato in contain per non tagliare le copertine e aggiunto background scuro */
    .gallery-main img {
        width: 100%;
        height: 100%;
        object-fit: contain;
    ...
    <?php if (!empty($table_disponibili)): ?>
        <?php foreach ($table_disponibili as $a):
            ...
            <div class="book-card" data-titolo="<?= htmlspecialchars(strtolower($a['titolo'] ?? '')) ?>"
                data-autore="<?= htmlspecialchars(strtolower($a['autore'] ?? '')) ?>" data-id="<?= $a['id_annuncio'] ?>"
                ...
                data-immagini="<?= htmlspecialchars($a['immagini'] ?? '') ?>" onclick="apriModalPref(this)">
                ...
                <a href="index.php?page=preferiti&action=store&id_annuncio=<?= $a['id_annuncio'] ?>" class="btn-rimuovi"
                    onclick="event.stopPropagation(); return confirm('Rimuovere dai preferiti?')">&#128465; Rimuovi dai
                    preferiti</a>
